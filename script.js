...
function handleRender(currTab) {
    ...
    for(const job in jobs) {
        ...
        if(currTab === "awaiting" || currJob.Status === currTab) {
            const jobCard = `
...
function tabChange(tab) {
    ...
    if((currTab === "awaiting" && jobCount === 0) || (currTab === "interview" && intJobCount === 0) || (currTab === "reject" && rejectJobCount === 0)) {
        noJobRender();
    } else {
        handleRender(currTab);
    }
